...
// ✅ ICONS
import { FaEdit, FaTrash, FaGolfBall, FaInfoCircle } from "react-icons/fa";
...
const ScoreCard = ({ score, onEdit, onDelete, index }) => {
  ...
  return (
    <motion.div
      ...
      exit={{ opacity: 0, x: -50 }}
      transition={{ delay: index * 0.05 }}
      className="bg-primary-800/60 border border-primary-700 rounded-2xl p-5 flex items-center justify-between group hover:border-primary-500 transition-all"
    >
      <div className="flex items-center gap-4">
        <div className="w-8 h-8 rounded-full bg-primary-700 flex items-center justify-center text-primary-400 text-xs font-bold">
          {index + 1}
      ...
      <div className="flex items-center gap-2">
        {confirmDelete ? (
          <div className="flex gap-2">
            <button
              onClick={() => onDelete(score._id)}
              className="px-3 py-1.5 bg-red-500/20 border border-red-500/40 text-red-400 text-xs rounded-lg"
            >
              Confirm
            </button>
            <button
              onClick={() => setConfirmDelete(false)}
              className="px-3 py-1.5 bg-primary-700 text-primary-300 text-xs rounded-lg"
            >
              Cancel
            </button>
          </div>
          ...
          <>
            <button
              onClick={() => onEdit(score)}
              className="p-2 rounded-xl text-primary-400 hover:bg-primary-700 hover:text-white opacity-0 group-hover:opacity-100 transition-all"
            >
              <FaEdit />
            </button>
            <button
              onClick={() => setConfirmDelete(true)}
              className="p-2 rounded-xl text-primary-400 hover:bg-red-500/20 hover:text-red-400 opacity-0 group-hover:opacity-100 transition-all"
            >
              <FaTrash />
            </button>
          </>
...
const StatsBar = ({ stats }) => {
  ...
  const trendIcon =
    stats.trend === 'up'
      ...
      : <BsArrowRight className="inline text-primary-400" />;
  ...
  return (
    ...
    >
      {[
        ...
        { label: 'Best', value: stats.highest, suffix: 'pts', color: 'text-green-400' },
        { label: 'Lowest', value: stats.lowest, suffix: 'pts', color: 'text-red-400' },
        { label: 'Trend', value: trendText, icon: trendIcon, isText: true },
      ].map(({ label, value, suffix, color, isText, icon }) => (
        <div key={label} className="bg-primary-800/60 border border-primary-700 rounded-xl p-4 text-center">
          <div className={`text-xl font-bold ${color || 'text-white'}`}>
            {isText ? (
              <span className="flex items-center justify-center gap-1">{icon} {value}</span>
            ) : (
              `${value} ${suffix}`
            )}
          </div>
...
const ScoresPage = () => {
  ...
  const handleSubmit = async (value, date) => {
    let result;
    if (editingScore) result = await updateScore(editingScore._id, value, date);
    else result = await addScore(value, date);
  ...
  const handleDelete = async (scoreId) => {
    const result = await deleteScore(scoreId);
    result.success ? toast.success('Score deleted.') : toast.error(result.message);
  };
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-8">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <button
          onClick={handleAddNew}
          className="flex items-center gap-2 px-4 py-2.5 bg-gold-500 text-primary-900 font-bold rounded-xl hover:bg-gold-400 transition-all text-sm"
        >
          <FaGolfBall /> Add Score
        </button>
      </div>
      ...
      {/* Info */}
      <motion.div className="bg-primary-800/30 border border-primary-700/50 rounded-xl p-4 mb-6 flex items-start gap-3">
        <FaInfoCircle className="text-primary-400 mt-0.5" />
        <div>
          <p className="text-primary-300 text-sm font-medium">Rolling 5-Score System</p>
          <p className="text-primary-500 text-xs mt-1">
            Only your 5 most recent scores are kept. Adding a 6th removes the oldest.
          </p>
        </div>
      </motion.div>
      ...
      {scores.length === 0 ? (
        <div className="text-center py-16">
          <FaGolfBall className="text-4xl mx-auto text-primary-500 mb-4" />
          <p className="text-white font-medium mb-2">No scores yet</p>
          <button
            onClick={handleAddNew}
            className="px-6 py-3 bg-gold-500 text-primary-900 font-bold rounded-xl"
          >
            Add Your First Score
          </button>
        </div>
