...
// ✅ ICONS
import { FaUser, FaLock, FaCreditCard, FaSignOutAlt } from "react-icons/fa";
...
const SettingsPage = () => {
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-8">
      ...
      {/* Profile */}
      <Section title="Profile" icon={<FaUser className="text-primary-400" />}>
        <div className="space-y-4">
          <div>
            <label className="text-primary-300 text-xs font-medium block mb-1">Full Name</label>
            <input
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                value={user?.email}
                disabled
                className="w-full pl-9 bg-primary-900/50 border border-primary-700 text-primary-500 rounded-xl px-4 py-2.5 text-sm cursor-not-allowed"
              />
            </div>
          </div>
      ...
      {/* Password */}
      <Section title="Change Password" icon={<FaLock className="text-primary-400" />}>
        <div className="space-y-3">
          {[
            { label: 'Current Password', value: currentPassword, set: setCurrentPassword },
            { label: 'New Password', value: newPassword, set: setNewPassword },
      ...
      {/* Subscription */}
      <Section title="Subscription & Billing" icon={<FaCreditCard className="text-primary-400" />}>
        <div className="space-y-4">
          ...
          <div className="flex flex-col gap-2">
            <button
              onClick={openPortal}
              disabled={subLoading}
              className="w-full py-2.5 rounded-xl bg-primary-700 text-white text-sm font-medium hover:bg-primary-600"
            >
              Manage Billing & Payment Methods
            </button>
            ...
            {sub?.status === 'active' && !sub?.cancelAtPeriodEnd && (
              <button
                onClick={handleCancel}
                disabled={subLoading}
                className="w-full py-2.5 rounded-xl border border-red-500/30 text-red-400 hover:bg-red-500/10"
              >
                Cancel Subscription
              </button>
            )}
            ...
            {sub?.cancelAtPeriodEnd && (
              <button
                onClick={handleResume}
                disabled={subLoading}
                className="w-full py-2.5 rounded-xl border border-green-500/30 text-green-400 hover:bg-green-500/10"
              >
                Resume Subscription
              </button>
            )}
      ...
      {/* Logout */}
      <Section title="Account" icon={<FaSignOutAlt className="text-primary-400" />}>
        <button
          onClick={() => { logout(); window.location.href = '/'; }}
          className="w-full py-2.5 rounded-xl bg-primary-700 text-white text-sm font-medium hover:bg-primary-600 flex items-center justify-center gap-2"
        >
